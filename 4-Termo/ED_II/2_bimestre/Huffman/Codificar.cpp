...
Tabela* criaNoTabela(char palavra[], int freq) {
    ...
    novo->codHuff[0] = '\0';  // Inicializa o c�digo Huffman como vazio
	    ...
	    while ((c = fgetc(arquivo)) != EOF) {
	        if (isEspaco(c)) {
	            if (i > 0) {
	                ...
	                i = 0;  // reseta o �ndice para a pr�xima palavra
	            }
	            char espaco[] = " ";
    ...
    if (*inicio != NULL || (*inicio)->prox != NULL) {
	    ...
	    while (*inicio != NULL) {
	        ...
	        *inicio = (*inicio)->prox; 
	        ...
	        // insere noAtual na lista ordenada na posi��o correta
...
// criar a lista de caixas a partir da tabela de frequ�ncias
...
//  inserir uma �rvore na lista de caixas de forma ordenada
...
HTree* gerarArvoreHuffman(Caixa **inicioCaixa) {
    while (*inicioCaixa != NULL && (*inicioCaixa)->prox != NULL) {
		...
		// remove as duas caixas com menor frequ�ncia
